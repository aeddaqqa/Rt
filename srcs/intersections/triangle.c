...
int		ft_triangle_intersect(t_object *triangle, t_ray *ray, double *tmin)
{
	...
	t.normal = normalize(cross(t.ca, t.ba));
	t.distance = dot(t.normal, triangle->point_a);
	t.x = vect_sub(ray->origin, triangle->point_a);
	i.a = -1.0 * dot(t.x, t.normal);
	i.b = dot(ray->direction, t.normal);
